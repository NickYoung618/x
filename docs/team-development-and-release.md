...
按最新分工，独立 3D 设备接口模拟由下位机同学负责；F/E/A–D 的采集模拟、算法模拟及业务能力由中台负责人协调实现。先检查同学是否已提供部分能力，避免重复。
...
首工位的明确交付与现场配合见 [S01 下位机工作派任书](assignments/s01-plc-work-order-20260917.md)。
该文档仍是 S01 联调的验收要求；下位机开发排期改按[全工位并行先行派任书](assignments/plc-all-stations-parallel-work-order-20260917.md)，不把 D1–D3 误读成“只能完成 S01 后再开始下一动作”。
