...
2026-09-15 查过的服务器为 Windows Server 2019、2 核/8GB，当时未发现可用 GPU、.NET SDK、
WebView2 或 GitHub runner；这些信息仅属于那台旧服务器。
用户后续改为在一台 Windows 电脑下载包验收，该电脑的系统、架构及运行环境须独立检查，
不能沿用旧服务器结论。2026-09-17 尚未收到该电脑的验收报告。
...
- 若需要自动部署测试，再注册仓库范围 runner，标签建议 `self-hosted, Windows, X64, gaode-staging`。
...
当前单台 Windows 电脑可先下载候选包手工启动验收脚本，保存报告，不依赖 runner。
完整交付方案见 [协作与发包指南](team-development-and-release.md)。
...
| 2 | 确认全模拟模式，建立工单与检测场景 | 测试环境已配置托盘码与两件/两面/AB 方案的映射；不在业务界面绕过扫码选方案 |
| 3 | 提交准备检测，模拟实体启动/夹紧就绪 | 只有一个 TrayRunId；就绪反馈新鲜；准备任务已保存 |
| 4 | 看公共准备、扫码匹配和第一面进度 | 初始 3D → F 托盘码 → 码与场景匹配唯一方案并冻结版本；随后 A1/A2/B1/B2；帧与身份关联 |
