...
下位机：本次 Ready/夹紧/联锁的采样与新鲜度；公共移至 3D 的动作编码、受理/完成/失败信号；OperationId 或兼容握手的关联方式；停止、超时、重连和未知动作对账。3D 合成端口、故障注入与轨迹由中台负责；真实设备供方/现场需给出相机型号、SDK/运行依赖、定位调用、点云/结果引用、SlotId 稳定规则、坐标系/单位、标定版本、姿态/置信度及超时。真机时中台、PLC 和 3D 三方轨迹须证明实体到位先于定位触发。未提交前保持抽象端口。
