...
下位机同学提供版本化点表、Ready/夹紧/互锁新鲜度、公共 3D 位与到位语义、动作关联、停止和重连对账及 PLC 独立轨迹。当前没有专职 3D 同学：中台负责合成 3D 进程、未来真实 SDK 适配及合同测试；设备供方/现场提供真实 SDK/设备、坐标系/单位、标定版本、样件和空盘规则，真机轨迹由中台适配器与现场共同核对。前端同学按状态/错误合同验证当前页面，WPF 壳到位后另测实际桌面。中台还负责唯一业务流程、日志、测试驱动、候选包来源和分层报告。未确认这些外部输入不阻止 G0–G4，但阻止 G6；SDK 单独到手的边界见 [3D 接入准备](3d-adapter-readiness.md)。
