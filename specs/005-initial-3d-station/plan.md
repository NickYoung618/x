...
- `backend/src/Inspection.Infrastructure/`：中台调用侧的设备适配、诊断/必要持久化；独立 PLC/3D 设备模拟由下位机同学交付，实际 SDK 另增量接入。
...
下位机同学提供版本化点表、Ready/夹紧/互锁新鲜度、公共 3D 位与到位语义、动作关联、停止和重连对账，以及**独立 PLC 与 3D 设备接口模拟及其轨迹**。中台负责两个接口的调用侧、未来真实 3D SDK 适配及合同测试；设备供方/现场提供真实 SDK/设备、坐标系/单位、标定版本、样件和空盘规则，真机轨迹由中台适配器与现场共同核对。前端同学按状态/错误合同验证当前页面，WPF 壳到位后另测实际桌面。中台还负责唯一业务流程、日志、测试驱动、候选包来源和分层报告。未确认这些外部输入不阻止调用侧代码和单元/Host 测试，但阻止 G2 独立进程联调与 G6 真机验收；SDK 单独到手的边界见 [3D 接入准备](3d-adapter-readiness.md)。

给下位机负责人的可转交任务与回执要求见[工作派任书](../../docs/assignments/s01-plc-work-order-20260917.md)。
