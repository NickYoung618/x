...
| Host + 独立 PLC + 3D 相机模拟三进程 S01 | BLOCKED | 下位机全工位合同/PLC/3D 相机模拟与中台合成定位器尚未交付；当前 Host 不注册设备端口，启用工程开关也返回 `ST01_PROVIDER_UNAVAILABLE` |
...
现有测试端口替身没有独立设备轨迹，因此不覆盖实际 Modbus 编解码、3D 相机模拟进程调用、合成帧到槽位的转换、真实重连时序或跨重启去重。下位机同学按[全工位派任书](../../docs/assignments/plc-all-stations-parallel-work-order-20260917.md)先交全量合同和软件；S01 使用其中的[本站验收切片](../../docs/assignments/s01-plc-work-order-20260917.md)。中台随后实现已签认的调用适配和合成定位器，以真实中台入口跑完整[矩阵](test-matrix.md)，核对 Host、PLC、3D 相机和定位转换轨迹并生成 S01 Windows 候选包。完成前 PR #7 保持 Draft，G2/G3 不判通过。
