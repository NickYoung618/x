...
- [ ] T007 复用 `MotionExecutionLane`；由中台增加可控的独立合成运动设备和 3D 进程，提供双槽样例、来源元数据及故障注入；不得调用业务状态机生成设备答案；合成/真实 Provider 共用定位合同且真实缺依赖时不得回退合成
...
- [ ] T018 与下位机负责人及设备供方/现场核对并记录点表、会话/动作关联、公共 3D 位、停止/对账、相机型号/SDK/坐标系/单位/标定及测试件预期；SDK 单独到手仅做适配/离线回放合同测试；任何真机准入缺项阻止真实设备动作，不阻止已证实的软件结论
...
依赖：T004→T005/T006→T007/T008→T009→T010–T012→T013–T015（软件合并）；T016→T017（Windows 同包），T018→T019→T020（真机）。首工位实现 PR 未完成 T004–T015 前保持 Draft；旧 V6、合成 S01、V1.3 真机、真实 SDK、WPF 与完整整盘各自独立报告。PR #4 已合并的是准备文档，T004–T020 需新 PR/报告承接。
